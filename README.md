...
### ローカルでダブルクリックして確認したい場合

`npm run build` で生成される`dist/`は`type="module"`のスクリプトを使うため、
`index.html`をブラウザで直接ダブルクリックして開く（`file://`で開く）と、
ブラウザの仕様で読み込みがブロックされ**真っ白な画面**になります。これはサーバー
（`http://`）経由で配信することを前提にしたビルドだからで、不具合ではありません。

サーバーを立てずにダブルクリックだけで確認したい場合は、以下で単一HTMLファイルの
確認用ビルドを生成してください（Service Worker・PWA機能は含みません。動作確認専用）:

```bash
npm run build:standalone   # dist-standalone/standalone.html を生成
```

生成された`dist-standalone/standalone.html`は、CSS・JSをすべて1ファイルに
埋め込んだ単一HTMLで、ダブルクリックしてそのままブラウザで開けます。
